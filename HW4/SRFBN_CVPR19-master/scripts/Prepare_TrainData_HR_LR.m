...
% SET data dir
sourcedir = '/media/intel/G/Hung/VRDL/HW4/SRFBN_CVPR19-master/dataset/training_hr_images';
savedir = '/media/intel/G/Hung/VRDL/HW4/SRFBN_CVPR19-master/dataset/Augment';
    ...
    if USE_AUG
        for angle = 0 : 2 : 2
            for downidx = 0 : 1 : length(downsizes)
                ...
                saveHRfile = [im_name '_rot' num2str(angle*90) '_ds' num2str(downidx) '_bicubic.png'];
                saveLRfile = [im_name '_rot' num2str(angle*90) '_ds' num2str(downidx) '_bicubic.png'];
                imwrite(image_HR, fullfile(saveHRpath, saveHRfile));    
                imwrite(image_LR, fullfile(saveLRpath, saveLRfile));

                image_LR = imresize(image_HR, 1/scale, 'nearest');
                saveHRfile = [im_name '_rot' num2str(angle*90) '_ds' num2str(downidx) '_nearest.png'];
                saveLRfile = [im_name '_rot' num2str(angle*90) '_ds' num2str(downidx) '_nearest.png'];
                imwrite(image_HR, fullfile(saveHRpath, saveHRfile));    
                imwrite(image_LR, fullfile(saveLRpath, saveLRfile));         

                image_LR = imresize(image_HR, 1/scale, 'bilinear');
                saveHRfile =  [im_name '_rot' num2str(angle*90) '_ds' num2str(downidx) '_bilinear.png'];
                saveLRfile = [im_name '_rot' num2str(angle*90) '_ds' num2str(downidx) '_bilinear.png'];
                imwrite(image_HR, fullfile(saveHRpath, saveHRfile));    
                imwrite(image_LR, fullfile(saveLRpath, saveLRfile));               
            end          
        end
